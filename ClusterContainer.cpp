 ...
 */
bool
eq(const Cluster *a, const Cluster *b)
{
	...
	list<Readpair>::const_iterator ait = areads.begin(),
		bit = breads.begin();
...
bool
cluscomp::operator() (const Cluster *a, const Cluster *b)
{
...
ClusterContainer::ClusterContainer() : max_id(-1) { }
...
size_t ClusterContainer::get_size() const { return clusters.size(); }

/* O(log(n)) insert */
void
ClusterContainer::add(Cluster *cl)
{
...
/* O(log(n)) removal */
void
ClusterContainer::remove(Cluster *cl)
{
...
/* O(1) tracking list */
void
ClusterContainer::set_dirty(Cluster *cl)
{
	if (dcl_filter[cl->get_id()] == false) {
#ifdef DEBUG
...
void
ClusterContainer::unset_dirty(Cluster *cl)
{
	dcl_filter[cl->get_id()] = false;
}

void
ClusterContainer::reset_dirty()
{
...
ClusterContainer::fix(list<ReadInfo*> R)
{
	...
	for (list<ReadInfo*>::iterator it = R.begin(); it != R.end(); ++it) {
		ReadInfo *ri = *it;
		for (ReadInfo::iterator it2 = ri->begin();
		     it2 != ri->end();
		     ++it2) {
			Cluster *c = it2->first;
...
bool
sortfn(const Cluster *a, const Cluster *b)
{
	bool a_same = a->is_same_chr();
	bool b_same = b->is_same_chr();
	if (a_same && b_same)
...
ClusterContainer::best()
{
	...
	list<Cluster*> ret;
	for (ClusterContainer::iterator it = range.first;
	     it != range.second; ++it)
